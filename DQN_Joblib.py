...
# torch.set_num_threads(96)  # 将线程数设置为96，等于CPU核数
...
k = 10  # 10
time2update = 4
MAX_EPISODES = 30  # 36 #40
MAX_EP_STEPS = 120  # 120
...
class DQN(object):
    ...
    # 并行化采样处理
    def process_sample(sample, state_dim):
        state = torch.FloatTensor(sample[:state_dim])
        action = torch.LongTensor(sample[state_dim: state_dim + 1].astype(int))
        reward = torch.FloatTensor(sample[-state_dim - 1: -state_dim])
        next_state = torch.FloatTensor(sample[-state_dim - 1: -1])
        is_end = sample[-1:]
        return state, action, reward, next_state, is_end
...
# 开始训练
for i in tqdm(range(MAX_EPISODES), desc="Training Episodes"):
    ...
    all_reward = 0
    for j in tqdm(range(MAX_EP_STEPS), desc=f"Episode {i+1}/{MAX_EPISODES}"):
        ...
        fError.write(str(i) + ':' + str(j) + str("%.4f" % (error[0][1])) + str("%.4f" % (error[0][2])) + ' ' + str(
            "%.4f" % (error[0][3])) + ' ' + str("%.4f" % (error[0][4])) + '\n')
        fError.flush()
        # 擅自修改处
        ...
        if j == MAX_EP_STEPS - 1:
            ...
            fLastStep.write(str(i) + ':' + str(j) + ' ' + str("%.4f" % (error[0][1])) + ' ' + str(
                "%.4f" % (error[0][2])) + ' ' + str("%.4f" % (error[0][3])) + ' ' + str("%.4f" % (error[0][4])) + '\n')
            fReward.flush()
            fLastStep.flush()

        if i == MAX_EPISODES - 1:
            fRecord.write(str(i) + ':' + str(j) + ' ' + str("%.4f" % (error[0][1])) + ' ' + str(
                "%.4f" % (error[0][2])) + ' ' + str("%.4f" % (error[0][3])) + ' ' + str("%.4f" % (error[0][4])) + '\n')
            fRecord.flush()
    ...
    if ((i % 5 == 0)):
        ...
        torch.save(dqn.target_Net.state_dict(), model_file_dir + '/target.zip')
        torch.save(dqn.eval_Net.state_dict(), model_file_dir + '/eval.pth')
